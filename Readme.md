# VLMによるリアルタイム分析アプリ
## 概要
本プロジェクトは、VLMを活用したリアルタイム動画分析アプリケーションです。 カメラから取得した映像ストリームから、動画圧縮の要となるキーフレーム（Iフレーム）を効率的に抽出。抽出されたフレームをVLM (Vision Language Model) で解析することで、リアルタイムでの高度な内容理解とGUIへの結果表示を行います。
OpenAIのgpt-4oやOllamaやllama.cppでホストしたローカルVLMと連携することができます。
GUIはStreamlitを用いて構築しています。
...
### 動画キャプチャ設定
- `CAMERA_SOURCE`: カメラソース - 使用するカメラまたはストリームのソース（インデックス番号またはURL）(デフォルト: 0)
